...
(function initSearchHistory() {
    const HISTORY_KEY = 'elora_search_history';
    const MAX_HISTORY = 8;
    ...
    function bindForms() {
        document.querySelectorAll('form[data-autocomplete-url] input[name="q"]').forEach(function (input) {
            if (input.__eloraACBound) return;
            input.__eloraACBound = true;

            const form = input.closest('form');
            const autocompleteUrl = form.dataset.autocompleteUrl;
            const container = input.closest('.elora-search-inner') || input.parentElement;
            if (!container) return;
            container.style.position = 'relative';

            const dropdown = document.createElement('div');
            dropdown.className = 'elora-ac-dropdown absolute left-[-12px] right-[-12px] top-[calc(100%+8px)] bg-white rounded-2xl shadow-2xl border border-gray-100 z-[300] hidden overflow-hidden';
            ...
            function bindHistoryEvents() {
                dropdown.querySelector('.elora-clear-hist')?.addEventListener('click', function (e) {
                    e.stopPropagation(); clearHistory(); hide();
                });
                dropdown.querySelectorAll('.elora-sh-item').forEach(function (el) {
            ...
            function bindResultEvents(kw) {
                ...
                dropdown.querySelector('.elora-see-all')?.addEventListener('mousedown', function (e) {
            ...
            function renderHistory() {
                ...
                const items = hist.map(function (t) {
                    return '<div class="elora-sh-item flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 cursor-pointer" data-term="' + safeText(t) + '">' +
                ...
                show('<div class="flex items-center justify-between px-4 py-2 border-b border-gray-100">' +
                    ...
                    '<button class="elora-clear-hist text-[12px] text-main hover:underline">Clear</button></div>' + items);
                bindHistoryEvents();
            }
            ...
            function renderResults(products, kw) {
                ...
                show('<div class="px-4 py-2 border-b border-gray-100"><span class="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Products</span></div>' +
                    rows +
                    '<a href="' + seeAll + '" class="elora-see-all flex items-center justify-center gap-1 px-4 py-2.5 text-[12px] font-semibold text-main border-t border-gray-100 hover:bg-gray-50 transition">' +
        ...
        document.addEventListener('click', function (e) {
            if (!e.target.closest('.elora-ac-dropdown') && !e.target.closest('form[data-autocomplete-url]')) {
                document.querySelectorAll('.elora-ac-dropdown').forEach(function (d) { d.classList.add('hidden'); });
            }
        });
    }
    ...
    document.addEventListener('livewire:navigated', function () {
        ...
        document.querySelectorAll('form[data-autocomplete-url] input[name="q"]').forEach(function (i) { i.__eloraACBound = false; });
        bindForms();
    });
})();
