// Manufacturing Request detail page: chat send/append, realtime admin
// messages via Echo, and the shared payment-gateway modal wired per
// payment-request row.
import { getEcho } from '../../core/echo.js';
import { openModal } from '../../core/modals.js';
import { on } from '../../core/events.js';

const context = JSON.parse(document.getElementById('manufacturing-request-context')?.textContent || '{}');
const chatRoot = document.getElementById('manufacturing-request-chat');
const chatForm = document.getElementById('mf-chat-form');
...
if (context.tenantId && context.requestId) {
    getEcho().then((echo) => {
        echo.private(`tenant.${context.tenantId}.manufacturing.${context.requestId}`)
...
document.addEventListener('click', (event) => {
    const button = event.target.closest('[data-mf-pay-request]');
    if (!button) {
        return;
    }

    const modal = document.getElementById('mf-pay-modal');
    if (!modal) {
        return;
    }

    const labelEl = modal.querySelector('[data-mf-pay-summary-label]');
    const amountEl = modal.querySelector('[data-mf-pay-summary-amount]');
    ...
    openModal('mf-pay-modal', { fill: { payment_request_id: button.dataset.paymentRequestId } });
});
