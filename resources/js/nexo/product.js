window.nexoOpenDeliveryModal = function () {
            var el = document.getElementById('nexo-delivery-modal-backdrop');
            if (el) {
        ...
        window.nexoCloseDeliveryModal = function () {
            var el = document.getElementById('nexo-delivery-modal-backdrop');
            if (el) {
        ...
        window.nexoOpenPrivacyModal = function () {
            var el = document.getElementById('nexo-privacy-modal-backdrop');
            if (el) {
                el.style.display = 'flex';
                document.body.style.overflow = 'hidden';
            }
        ...
        window.nexoClosePrivacyModal = function () {
            var el = document.getElementById('nexo-privacy-modal-backdrop');
            if (el) {
                el.style.display = 'none';
                document.body.style.overflow = '';
        ...
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Escape') {
                window.nexoClosePrivacyModal();
                window.nexoCloseDeliveryModal();
            }
        });
        ...
        function nexoStopAllVideos(swiperEl, videoClass) {
        ...
        function nexoGoToMediaIndex(index) {
            if (index === null || index === undefined) return;
            if (window.nexoMainSwiper) window.nexoMainSwiper.slideToLoop(index);
            if (window.nexoFsSwiper) window.nexoFsSwiper.slideToLoop(index);
        }
        window.nexoGoToMediaIndex = nexoGoToMediaIndex;
        ...
        const swiper = new Swiper('.product-preview-swiper', {
            ...
            on: {
                slideChange: function () {
                    nexoStopAllVideos(this.el, 'nexo-gallery-video');
                    const activeSlide = this.slides[this.activeIndex];
                    if (activeSlide) {
                        const vid = activeSlide.querySelector('.nexo-gallery-video');
                        if (vid) {
                            vid.play().catch(() => {
                            });
                        }
        ...
        const fsSwiper = new Swiper('.fs-product-preview-swiper', {
            ...
            on: {
                slideChange: function () {
                    nexoStopAllVideos(this.el, 'nexo-fs-gallery-video');
                    const activeSlide = this.slides[this.activeIndex];
                    if (activeSlide) {
                        const vid = activeSlide.querySelector('.nexo-fs-gallery-video');
                        if (vid) {
                            vid.play().catch(() => {
                            });
                        }
                ...
                if (typeof Livewire !== 'undefined') {
                    Livewire.on('storefront-cart-added', ({
                        ...
                        if (typeof showCartToast === 'function') {
                            showCartToast(itemName || (window.__nexoProductConfig && window.__nexoProductConfig.textItem) || window.trans('Item'), qty || 1);
                        }
                    });
                }
            }
        ...
        (function () {
            ...
            window.nexoOpenShareModal = function () {
                var cfg = window.__nexoProductConfig || {};
                shareUrl = cfg.shareUrl || window.location.href;
                var modal = document.getElementById('nexo-share-modal');
                var sheet = document.getElementById('nexo-share-sheet');
                var urlText = document.getElementById('nexo-share-url-text');
                if (!modal) return;
                ...
                document.getElementById('nexo-share-whatsapp').href = 'https://wa.me/?text=' + shareText + '%20' + encoded;
                document.getElementById('nexo-share-facebook').href = 'https://www.facebook.com/sharer/sharer.php?u=' + encoded;
                document.getElementById('nexo-share-twitter').href = 'https://twitter.com/intent/tweet?url=' + encoded + '&text=' + productName;
                document.getElementById('nexo-share-telegram').href = 'https://t.me/share/url?url=' + encoded + '&text=' + shareText;
            ...
            window.nexoCloseShareModal = function () {
                var modal = document.getElementById('nexo-share-modal');
                var sheet = document.getElementById('nexo-share-sheet');
                if (!modal) return;
            ...
            window.nexoCopyShareUrl = function () {
                if (!shareUrl) return;
                var btn = document.getElementById('nexo-copy-btn');
                var textCopied = window.__nexoProductConfig && window.__nexoProductConfig.textCopied || window.trans('Copied!');
                var textCopy   = window.__nexoProductConfig && window.__nexoProductConfig.textCopy   || window.trans('Copy');
                function showCopied() {
                    if (btn) {
            ...
            document.addEventListener('keydown', function (e) {
                if (e.key === 'Escape') window.nexoCloseShareModal();
            });
        })();


        (function () {
            var STORAGE_KEY = 'nexo_favorites';
            ...
            function syncHeartUI(favorited) {
                var outline = document.getElementById('nexo-fav-icon-outline');
                var filled  = document.getElementById('nexo-fav-icon-filled');
                var btn     = document.getElementById('nexo-fav-btn');
            ...
            window.nexoHeartToggle = function (btn) {
                ...
                if (!isLoggedIn) {
                    if (typeof showStorefrontToast === 'function') {
                        showStorefrontToast(window.__nexoProductConfig && window.__nexoProductConfig.textLoginFavorites || window.trans('Please log in to add products to your favorites.'), 'warning');
                    }
                    return;
                }
                ...
                if (idx !== -1) {
                    ...
                    syncHeartUI(false);
                    if (typeof showStorefrontToast === 'function') {
                        showStorefrontToast(window.__nexoProductConfig && window.__nexoProductConfig.textRemovedFav || window.trans('Removed from favorites.'), 'info');
                    }
                } else {
                    // Not favorited → add
                    try {
                        ...
                        syncHeartUI(true);
                        if (typeof showStorefrontToast === 'function') {
                            showStorefrontToast(window.__nexoProductConfig && window.__nexoProductConfig.textAddedFav || window.trans('Added to favorites!'), 'success');
                        }
                    } catch (e) {
                        console.error('nexoHeartToggle: failed to parse fav data', e);
                    }
                }
            };
