...
@php
    ...
    // Helper: get old input with fallback to $productData, then default
    // Usage in blade: $f('field', 'fallback')
    ...
    <form id="product-form"
        ...
        <div class="grid gap-2">
            ...
            <x-card-collapse title="Basics"
                subtitle="Identifiers, publication state, factory origin, and weight."
                ...
                <div class="form-grid form-grid-3">
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                                class="field-control {{ $errors->has('status') ? 'is-invalid' : '' }}">
                            @foreach ($statusOptions as $opt)
                                <option value="{{ $opt->value }}"
                                    {{ $f('status', 'draft') === $opt->value ? 'selected' : '' }}>
                                    {{ $opt->label() }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input id="factory" name="factory" type="text"
                               class="field-control {{ $errors->has('factory') ? 'is-invalid' : '' }}"
                               value="{{ $f('factory') }}"
                               placeholder="e.g. FAC-023 or Shenzhen Co.">
            ...
            <x-card-collapse title="Primary Image"
                subtitle="Main product image. Generates large, medium, and thumb derivatives."
                    ...
                    <div class="w-full relative group" data-dropzone>
                        <label class="relative flex flex-col items-center justify-center w-full py-4 px-4 border-2 border-dashed border-(--border2) rounded-xl bg-(--surface) hover:border-(--cyan) transition-all duration-300 cursor-pointer focus-within:ring-2 focus-within:ring-(--cyan) focus-within:ring-offset-2 overflow-hidden">
                            ...
                            <div class="relative text-center z-10">
                                ...
                                <p class="text-[11px] text-(--t3)">PNG, JPG, WEBP up to 4MB</p>
                            </div>
            ...
            <x-card-collapse title="Media Gallery"
                ...
                {{-- Container where remove_gallery_ids hidden inputs are injected --}}
            ...
            <x-card-collapse class="form-card span-6" title="Categories"
                ...
                @php
                    $catMap = [];
                    $buildCatMap = function ($items, $parentId = null) use (&$buildCatMap, &$catMap) {
                        foreach ($items as $cat) {
                            $hasChildren = $cat->children && $cat->children->isNotEmpty();
                            $catMap[(int) $cat->id] = [
                                'id'           => (int) $cat->id,
                                'name'         => (string) ($cat->translationValue('name') ?? $cat->slug),
                                'parent_id'    => $parentId !== null ? (int) $parentId : null,
                                'has_children' => $hasChildren,
                            ];
                            if ($hasChildren) {
                                $buildCatMap($cat->children, $cat->id);
                            }
                        }
                    };
                    $buildCatMap($categoryTree ?? collect());
                    $selectedCategoryIds = array_map('intval', old('category_ids', $productData['category_ids'] ?? []));
                @endphp
                ...
                (function () {
                    ...
                    var catPath     = [];  // IDs navigated so far (navigation state, not final selections)
            ...
            <x-card-collapse class="form-card span-12" title="Product Variants"
                ...
                <div class="notice-muted" style="margin-bottom:16px">
                    ...
                    (Size, Color, …), and set this variant's own price, stock and thumbnail image.
                ...
                {{-- Encoded variation data for JS --}}
                @php
                    $json = $variations->keyBy('id')->map(fn($v) => [
                ...
                <div id="variants-list">
                    @foreach ($variants as $vIdx => $variant)
                        @include('admin.product._variant-row', [
                            'vIdx'       => $vIdx,
                            'variant'    => $variant,
            ...
            <x-card-collapse title="Tenant Assignments"
                ...
                @else
                    @php
                        $selectedTenantIds = old('assigned_tenant_ids', $assignedTenantIds ?? []);
                        $allTenantsChecked = old('assign_to_all_tenants', $assignToAllTenants ?? false);
                    @endphp
                    <div x-data="{ allTenants: {{ $allTenantsChecked ? 'true' : 'false' }} }">
                        <label style="display:flex;align-items:center;gap:8px;margin-bottom:12px;cursor:pointer;">
                            <input type="checkbox" name="assign_to_all_tenants" value="1"
                                x-model="allTenants"
                                {{ $allTenantsChecked ? 'checked' : '' }}
                                style="width:16px;height:16px;">
                            <span style="font-weight:500;">Assign to all tenants</span>
                        </label>
                        <div x-show="!allTenants">
                            <x-select multiple searchable name="assigned_tenant_ids[]" placeholder="Search and select tenants">
                                @foreach ($tenants as $tenant)
                                    <option value="{{ $tenant->id }}"
                                        {{ in_array($tenant->id, $selectedTenantIds) ? 'selected' : '' }}>
                                        {{ $tenant->name ?: $tenant->id }}
                                    </option>
                                @endforeach
                            </x-select>
                            @error('assigned_tenant_ids') <p class="field-error" style="margin-top:8px">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div class="notice-muted" style="margin-top:8px">
                        Newly assigned tenants will receive a notification. Removed tenants will lose access to this product.
                    </div>
                @endif
            </x-card-collapse>
...
(function () {
    ...
    // Counter starts after server-rendered variants
    var nextVariantIdx = {{ count($variants) }};

    function buildVariantRow(vIdx, variantId, price, stock, sku, weightGrams, thumbnailUrl) {
        variantId    = variantId    || '';
        price        = price        || '';
        stock        = stock        || '0';
        sku          = sku          || '';
        ...
        Object.keys(__variations).forEach(function (id) {
            variationOptions += '<option value="' + id + '">'
                + escHtml(__variations[id].name) + '</option>';
        });

        var thumbHtml = thumbnailUrl
            ? '<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">'
              +   '<img id="vthumb-' + vIdx + '" src="' + escHtml(thumbnailUrl) + '" alt="" style="width:64px;height:64px;object-fit:cover;border-radius:6px;border:1px solid #ddd">'
              +   '<button type="button" class="btn btn-secondary btn-sm btn-danger v-img-remove-btn" onclick="removeVariantImage(this,' + vIdx + ')">Remove image</button>'
              + '</div>'
            : '';
        ...
        return '<div class="vgroup-card" data-vcard>'
            ...
            +     '<input type="text" name="variants[' + vIdx + '][title]" class="field-control" value="" placeholder="Auto-generated from options if left blank">'
            +   '</div>'
            ...
            +       '<svg width="11" height="11" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">'
            +         '<line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/>'
            +       '</svg> Add another group'
            +     '</button>'
            +   '</div>'
            +   '<div class="form-grid form-grid-3" style="margin-top:14px">'
            +     '<div><label class="field-label">Price *</label>'
            +       '<input type="number" step="0.01" min="0" name="variants[' + vIdx + '][price]" value="' + escHtml(price) + '" class="field-control"></div>'
            +     '<div><label class="field-label">Stock</label>'
            +       '<input type="number" min="0" step="1" name="variants[' + vIdx + '][stock]" value="' + escHtml(stock) + '" class="field-control"></div>'
            +     '<div><label class="field-label">SKU (optional)</label>'
            +       '<input type="text" name="variants[' + vIdx + '][sku]" value="' + escHtml(sku) + '" placeholder="Auto-generated if blank" class="field-control"></div>'
            +     '<div><label class="field-label">Weight (grams)</label>'
            +       '<input type="number" min="0" step="1" name="variants[' + vIdx + '][weight_grams]" value="' + escHtml(weightGrams) + '" placeholder="0" class="field-control"></div>'
            +   '</div>'
            + '</div>'
            + '</div>';
    }
    ...
    window.removeVariantImage = function (btn, vIdx) {
        var card = btn.closest('[data-vcard]');
        // Hide the existing DB thumbnail + its remove button
        var imgWrap = btn.closest('[style*="display:flex"]') || btn.parentElement;
        if (imgWrap) imgWrap.style.display = 'none';
        // Clear the dropzone (if any file was selected)
    ...
    function getVariantIdx(card) {
        var allCards = document.querySelectorAll('#variants-list [data-vcard]');
        return Array.prototype.indexOf.call(allCards, card);
    }

    function renumberVariants() {
        document.querySelectorAll('#variants-list [data-vcard]').forEach(function (card, vIdx) {
            card.querySelectorAll('[name]').forEach(function (el) {
                el.name = el.name.replace(/variants\[\d+\]/g, 'variants[' + vIdx + ']');
            });
            // Update thumb id
            var thumb = card.querySelector('[id^="vthumb-"]');
            if (thumb) thumb.id = 'vthumb-' + vIdx;
            // Update remove-image button onclick
            var removeImgBtn = card.querySelector('.v-img-remove-btn');
            if (removeImgBtn) {
                removeImgBtn.setAttribute('onclick', 'removeVariantImage(this,' + vIdx + ')');
            }
            renumberPairs(card);
        });
        updateVariantNumbers();
    ...
    function renumberPairs(card) {
        ...
        card.querySelectorAll('.variant-pair-row').forEach(function (row, pIdx) {
            row.querySelectorAll('[name]').forEach(function (el) {
                el.name = el.name.replace(/pairs\[\d+\]/g, 'pairs[' + pIdx + ']');
                el.name = el.name.replace(/variants\[\d+\]/g, 'variants[' + vIdx + ']');
            });
            // Show/hide remove button on first pair
            var removeBtn = row.querySelector('.btn-danger');
            if (removeBtn) removeBtn.style.display = pIdx === 0 ? 'none' : '';
        });
    }

    function updateVariantNumbers() {
        document.querySelectorAll('#variants-list [data-vcard]').forEach(function (card, idx) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        initTinyMCE();
        updateVariantNumbers();
        // Initialize pair remove buttons for server-rendered variants
        document.querySelectorAll('#variants-list [data-vcard]').forEach(function (card) {
            renumberPairs(card);
        });
    });
    ...
    (function () {
        ...
        var validateUrl = @json($isEdit ? route('admin.products.validate.update', $product) : route('admin.products.validate'));
