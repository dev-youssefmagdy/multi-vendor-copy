<header>
  ...
  >
    <div class="flex items-center justify-between gap-[8px]">
      <div class="flex items-center gap-[12px]">
        ...
        >
          ...
          <span class="text-[8px] tracking-[0.3px]" style="color: var(--color-black)">menu</span>
        </button>
        <img src="{{ $logoPath ?? asset('elora-5/assets/icons/logo-elora.svg') }}" alt="{{ $storeName ?? 'ELORA' }}" class="h-[26px] w-auto" />
      </div>
      <button
        type="button"
    ...
    <div class="flex items-center gap-[8px]">
      <div
        ...
        style="background: var(--color-page-bg); border-color: var(--color-stroke);"
      >
        <img src="{{ asset('elora-5/assets/icons/search.svg') }}" alt="" class="size-[18px] opacity-70" />
        <input type="search" placeholder="Search..." class="bg-transparent outline-none text-[15px] w-full" style="color: var(--color-gray)" />
      </div>
      <button
        type="button"
        ...
        style="background: var(--color-page-bg)"
      >
        <img src="{{ asset('elora-5/assets/icons/icon-camera.svg') }}" alt="" class="size-[18px]" />
      </button>
    </div>
  </div>
  ...
  <div class="hidden lg:flex items-center gap-[18px] px-[48px] py-[18px] bg-white">
    ...
    <img src="{{ $logoPath ?? asset('elora-5/assets/icons/logo-elora.svg') }}" alt="{{ $storeName ?? 'ELORA' }}" class="h-[38px] w-auto" />
    <div class="flex flex-1 items-center gap-[8px] h-[54px] rounded-[32px] px-[24px]" style="background: var(--color-page-bg)">
      <img src="{{ asset('elora-5/assets/icons/search.svg') }}" alt="" class="size-[22px] opacity-70" />
    ...
    <div class="flex items-center gap-[38px] shrink-0">
      <div class="flex items-center gap-[6px]">
        <img src="{{ asset('elora-5/assets/icons/flag-en.png') }}" alt="EN" class="h-[25px] w-[40px] object-cover rounded-[2px]" />
        <div class="flex flex-col">
          <span class="font-light text-[12px] tracking-[0.5px]" style="color: var(--color-black-alt)">En/</span>
          <span class="font-medium text-[12px] tracking-[0.5px] flex items-center gap-[2px]" style="color: var(--color-black-alt)"
            >SAR
            <img src="{{ asset('elora-5/assets/icons/arrow-down.svg') }}" class="size-[12px]" alt="" />
          </span>
        </div>
      </div>
      ...
      <button type="button" class="flex items-center gap-[8px] cursor-pointer" aria-label="Account">
        ...
        <span class="text-[14px] tracking-[0.5px] leading-tight text-left">
          <span class="block" style="color: var(--color-gray)">Welcome</span>
...
<div id="mobileDrawerOverlay" class="mobile-drawer-overlay fixed inset-0 bg-black/50 z-40"></div>
<aside
  id="mobileDrawer"
...
>
  ...
  <div class="flex items-center gap-[8px] mx-[16px] mt-[16px] h-[44px] rounded-[24px] px-[16px]" style="background: var(--color-page-bg)">
    <img src="{{ asset('elora-5/assets/icons/search.svg') }}" alt="" class="size-[18px] opacity-70" />
    <input type="search" placeholder="Search..." class="bg-transparent outline-none text-[14px] w-full" style="color: var(--color-gray)" />
  </div>
  <nav class="flex flex-col px-[16px] py-[12px] overflow-y-auto no-scrollbar">
    @if (!empty($rootCategories))
      @foreach ($rootCategories as $index => $category)
        <a
          href="{{ $category->url ?? '#' }}"
          class="flex items-center justify-between py-[12px] border-b text-[15px] tracking-[0.3px] {{ $index === 0 ? 'font-medium' : '' }}"
          style="border-color: var(--color-page-bg); color: {{ $index === 0 ? 'var(--color-primary)' : 'var(--color-black)' }};"
          >{{ $category->name ?? $category }}
          <img src="{{ asset('elora-5/assets/icons/arrow-down.svg') }}" class="size-[14px] -rotate-90 {{ $index === 0 ? 'opacity-60' : 'opacity-40' }}" alt="" />
        </a>
      @endforeach
    @else
      @foreach (["Women's Bags", 'Accessories', 'Gaming', 'Electronics', 'Fashion', 'Watches'] as $index => $label)
        <a
          href="#"
          class="flex items-center justify-between py-[12px] {{ !$loop->last ? 'border-b' : '' }} text-[15px] tracking-[0.3px] {{ $index === 0 ? 'font-medium' : '' }}"
          style="border-color: var(--color-page-bg); color: {{ $index === 0 ? 'var(--color-primary)' : 'var(--color-black)' }};"
          >{{ $label }}
          <img src="{{ asset('elora-5/assets/icons/arrow-down.svg') }}" class="size-[14px] -rotate-90 {{ $index === 0 ? 'opacity-60' : 'opacity-40' }}" alt="" />
        </a>
