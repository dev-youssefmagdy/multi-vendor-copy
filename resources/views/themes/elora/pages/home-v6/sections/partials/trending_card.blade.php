{{-- Expects $p: name, weight, desc, badge, badgeBg, badgeColor, progress, ordered, rating, price, oldPrice, discount, image, url --}}
...
<a href="{{ $p['url'] ?? '#' }}" class="flex h-auto bg-[var(--color-bg-main)] rounded-[6px] lg:rounded-[10.1143px] shadow-sm overflow-hidden no-underline">
  ...
  <div class="flex-1 px-2 py-1 lg:px-[13.486px] lg:py-[6.743px] flex flex-col gap-2 lg:gap-[13.49px] min-w-0">
    <div class="flex flex-col gap-[2.51px] lg:gap-[13.49px]">
      ...
      <p class="text-[12px] lg:text-[20.23px] leading-[15px] lg:leading-[25px] tracking-[0.314px] lg:tracking-[0.53px] truncate" style="color:var(--color-text-subtitle)">{{ $p['desc'] ?? __('Premium cotton blend') }}</p>
    </div>
