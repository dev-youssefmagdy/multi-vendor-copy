    ...
    >
      <div class="relative flex items-center gap-[10px]">
        <img src="{{ asset('elora-5/assets/icons/icon-flash.svg') }}" alt="" class="size-[28px] lg:size-[40px]" />
        <h2 class="font-semibold text-[26px] lg:text-[42px] text-white tracking-[0.5px]">Flash Sale</h2>
        <img src="{{ asset('elora-5/assets/icons/icon-flash.svg') }}" alt="" class="size-[28px] lg:size-[40px]" />
      </div>
      <div class="relative w-full">
        <div class="swiper card-swiper flash-swiper">
          <div class="swiper-wrapper" id="flashSaleWrapper">
            <div class="swiper-slide !w-[200px] lg:!w-[298px] !h-[302px] lg:!h-[450px]">
              @include('themes.elora.pages.home-v5.sections.partials.fan_card', ['p' => $flashProducts[0]])
            </div>
            <div class="swiper-slide !w-[362px] lg:!w-[536px] !h-[302px] lg:!h-[450px]">
              <div class="flex flex-col gap-[10px] lg:gap-[12px] items-start h-full">
                ...
                @include('themes.elora.pages.home-v5.sections.partials.flash_mini_card', ['p' => $flashProducts[1]])
                @include('themes.elora.pages.home-v5.sections.partials.flash_mini_card', ['p' => $flashProducts[2]])
              </div>
            </div>
            @foreach ([$flashProducts[3], $flashProducts[4], $flashProducts[5]] as $product)
              <div class="swiper-slide !w-[200px] lg:!w-[298px] !h-[302px] lg:!h-[450px]">
        ...
        <button id="flashSalePrev" type="button" aria-label="Previous" class="swiper-nav-btn swiper-nav-prev">
          <img src="{{ asset('elora-5/assets/icons/arrow-down.svg') }}" class="size-[14px] rotate-90" alt="" />
        </button>
        <button id="flashSaleNext" type="button" aria-label="Next" class="swiper-nav-btn swiper-nav-next">
          <img src="{{ asset('elora-5/assets/icons/arrow-down.svg') }}" class="size-[14px] -rotate-90" alt="" />
        </button>
      </div>
      <button type="button" class="relative border border-white rounded-full h-[48px] lg:h-[64px] px-[32px] flex items-center justify-center cursor-pointer">
        <span class="font-medium text-white text-[16px] lg:text-[20px] tracking-[0.5px]">Shop now</span>
      </button>
    </section>
