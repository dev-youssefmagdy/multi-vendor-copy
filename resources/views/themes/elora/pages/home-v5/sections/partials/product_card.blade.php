...
<a href="{{ $p['url'] ?? '#' }}" class="flex flex-col items-start rounded-[8px] h-full shadow-[var(--shadow-card-lg)]" style="background:var(--color-bg-main)">
  <div class="relative flex flex-col gap-[4px] lg:gap-[6px] h-[130px] lg:h-[190px] items-end justify-end px-[5px] lg:px-[7px] py-[4px] lg:py-[5px] shrink-0 w-full">
    <div class="absolute left-0 top-0 h-full w-full rounded-t-[6px] lg:rounded-t-[8px] overflow-hidden">
    ...
    <div class="absolute top-0 left-0 flex items-center justify-center px-[10px] py-[5px] rounded-bl-[8px] rounded-tr-[6px] lg:rounded-tr-[8px] shrink-0" style="background:{{ !empty($p['alt']) ? 'var(--color-badge-orange)' : 'var(--color-yellow)' }}">
    ...
    @if (!empty($p['favData']))
      <button type="button" aria-label="{{ __('Add to favorites') }}" onclick="event.preventDefault(); event.stopPropagation(); eloraHeartToggle(this)"
        ...
        class="absolute top-[5px] right-[5px] bg-white cursor-pointer shadow flex items-center justify-center p-[5px] lg:p-[8px] rounded-full shrink-0 size-[22px] lg:size-[32px]">
        <img src="{{ asset('elora-5/assets/icons/heart.svg') }}" alt="" class="size-[13px] lg:size-[19px]" />
      </button>
    @else
      <button type="button" aria-label="{{ __('Add to favorites') }}" class="absolute top-[5px] right-[5px] bg-white cursor-pointer shadow flex items-center justify-center p-[5px] lg:p-[8px] rounded-full shrink-0 size-[22px] lg:size-[32px]">
        <img src="{{ asset('elora-5/assets/icons/heart.svg') }}" alt="" class="size-[13px] lg:size-[19px]" />
      </button>
    @endif
