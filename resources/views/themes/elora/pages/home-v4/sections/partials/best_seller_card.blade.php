...
<div class="swiper-slide h-auto !w-[193px]">
  <a href="{{ $p['url'] ?? '#' }}" class="flex flex-col items-start rounded-[9px] shadow-[var(--shadow-card-lg)]" style="background:var(--color-bg-main); text-decoration:none">
    <div class="flex flex-col gap-[7px] h-[167px] items-end justify-end px-[5px] py-[5px] relative shrink-0 w-full">
      <div class="absolute flex gap-[7px] h-[167px] items-start left-0 p-[5px] top-0 w-full">
        ...
        <button type="button" onclick="event.preventDefault(); event.stopPropagation(); eloraV4ToggleFavorite(this)"
          data-fav='{{ $p['favData'] ?? '{}' }}'
          aria-label="{{ __('Add to favorites') }}" class="elora-v4-heart-btn bg-white cursor-pointer shadow flex items-center justify-center p-[7px] relative rounded-full shrink-0 size-[29px]">
      ...
      <button type="button" wire:click.prevent="addToCart({{ $p['id'] ?? 0 }})" onclick="event.stopPropagation()"
        ...
        class="flex items-center justify-center px-[11px] py-[4px] relative rounded-[15px] shrink-0 h-[41px] w-[52px] cursor-pointer" style="background:var(--color-text-primary)">
        <img src="{{ asset('elora-4/assets/icons/cart.svg') }}" alt="" class="size-[22px] invert" />
      </button>
    </div>
