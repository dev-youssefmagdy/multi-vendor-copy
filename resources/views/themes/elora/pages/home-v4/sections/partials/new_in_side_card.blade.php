...
<a href="{{ $p['url'] ?? '#' }}" class="bg-[var(--color-bg-main)] flex items-stretch rounded-[6px] lg:rounded-[8px] h-full shadow-[var(--shadow-card-lg)] overflow-hidden" style="text-decoration:none">
  <div class="relative shrink-0 w-[36%]">
    ...
    <button type="button" onclick="event.preventDefault(); event.stopPropagation(); eloraV4ToggleFavorite(this)"
      data-fav='{{ $p['favData'] ?? '{}' }}'
      aria-label="{{ __('Add to favorites') }}" class="elora-v4-heart-btn absolute top-[5px] left-[5px] lg:top-[8px] lg:left-[8px] bg-white cursor-pointer shadow flex items-center justify-center p-[4px] lg:p-[6px] rounded-full size-[18px] lg:size-[26px]">
    ...
    <button type="button" wire:click.prevent="addToCart({{ $p['id'] ?? 0 }})" onclick="event.stopPropagation()"
      ...
      class="absolute bottom-[5px] right-[5px] lg:bottom-[8px] lg:right-[8px] flex items-center justify-center rounded-[8px] lg:rounded-[12px] h-[20px] lg:h-[32px] px-[5px] lg:px-[9px] cursor-pointer" style="background:var(--color-text-primary)">
      <img src="{{ asset('elora-4/assets/icons/cart.svg') }}" alt="" class="size-[11px] lg:size-[17px] invert" />
    </button>
  </div>
