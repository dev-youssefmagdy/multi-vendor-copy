...
<main class="flex-grow bg-white pt-8 w-full">
    <div class="max-w-[1280px] mx-auto px-5 sm:px-8 mb-6">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div class="flex items-center flex-wrap gap-4">
                ...
                @if ($isSearchRoute)
                    <div class="flex items-center gap-2 text-[13px] text-[#888]">
                        {{ __('Matching products across the store') }}
                    </div>
                @else
                    <div class="flex items-center gap-3 overflow-x-auto whitespace-nowrap scroll-smooth no-scrollbar pb-1 -mb-1 max-w-full"
                        ...
                        @foreach ($dayOptions as $option)
                            <a href="{{ route('tenant.storefront.best-selling', array_filter([
                                ...
                                class="px-5 py-1.5 rounded-full border {{ $days === $option ? 'border-[#222] text-[#222] font-semibold' : 'border-[#e5e5e5] text-[#888] font-medium' }} text-[13px] bg-white hover:bg-gray-50 transition shrink-0">
            ...
            <form action="{{ $searchAction }}" method="GET" class="flex items-center gap-3 flex-wrap" style="position: relative;">
                @if (!$isSearchRoute)
                ...
                <span class="text-[#888] text-[13px] font-medium">{{ __('Filter by category') }}</span>
                ...
                <button type="submit"
                    class="px-4 py-1.5 rounded-full border border-[#222] text-[#222] font-semibold text-[13px] bg-white hover:bg-gray-50 transition">{{ __('Search') }}</button>
            </form>
        </div>

        @include('themes.ecommet.pages._categories-slider', [
            'allLabel' => __('All categories'),
            'allHref' => $isSearchRoute
            ...
            'items' => $categories->map(fn($category) => [
                'href' => $isSearchRoute
                    ? route('tenant.storefront.search', array_filter([
                        'q' => $search !== '' ? $search : null,
                        'category_id' => $category->id,
                    ]))
                    : route('tenant.storefront.best-selling', array_filter([
                        'days' => $days,
                        'keyword' => $search !== '' ? $search : null,
                        'category_id' => $category->id,
                    ])),
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
                @foreach ($products as $product)
                    @include('themes.ecommet.pages._showcase-card', [
                        'product' => $product,
                        'badgeText' => __('Best-Selling'),
                        'badgeColorClass' => 'bg-[#E01911]',
