{{-- Header/Footer partial for nexo theme --}}
...
@endphp
<script>
    window.__nexoCatTree = @json($buildCatTree($rootCategories));
</script>
<header class="navbar">
    {{-- <!-- Announcement -->--}}
    ...
    <div class="bg-white px-3 sm:px-6 lg:px-10 py-2.5 sm:py-3 flex items-center gap-2 sm:gap-3">
        <button id="menuBtn"
            onclick="openSideMenu(); setTimeout(function(){ var t=window.__nexoCatTree; window.nexoActivateCat && t && t.length && window.nexoActivateCat(t[0].id); }, 30);"
        ...
        <div class="flex-1 min-w-0 mx-1 sm:mx-3 lg:mx-4">
            <form action=" {{ route('tenant.storefront.search') }}" method="GET"
                ...
                <div class="nexo-search-inner flex items-center bg-bg rounded-full px-3 sm:px-5 py-2 gap-2 h-14"
    ...
    <div id="sideMenu" class="hidden relative" style="z-index:400">
        ...
        <div class="menu-panel !absolute !h-[calc(100vh-108px)] shadow-2xl" id="nexoSideMenuPanel">
            <div class="flex flex-row" style="height:100%;">

                {{-- ── Left column: parent categories ── --}}
                <div class="flex flex-col overflow-y-auto bg-white" id="nexoCatLeft"
                    ...
                    @forelse ($rootCategories as $category)
                        <button type="button"
                            ...
                            onmouseenter="nexoActivateCat({{ $category->id }})" onclick="nexoHandleCatClick(this)">
                ...
                <div class="flex flex-col overflow-y-auto" id="nexoCatRight"
                    style="width:50%; background:#F4F4F4; padding:0; gap:0; box-sizing:border-box;">
                    <div id="nexoCatRightContent"
...
{{-- ── Nexo side-menu two-column styles ── --}}
<style>
    #nexoSideMenuPanel {
...
<script>
    (function () {
        ...
        function renderRight() {
            var content = document.getElementById('nexoCatRightContent');
            if (!content) return;
            ...
            if (rightStack.length > 1) {
                html += '<button type="button" class="nexo-right-back-btn" onclick="window.nexoRightBack()" ' +
                    ...
                    '<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" class="nexo-right-back-arrow" style="flex-shrink:0;">' +
            ...
            level.items.forEach(function (cat) {
                ...
                if (hasChildren) {
                    ...
                    html += '<div style="display:flex;align-items:center;border-radius:4px;min-height:34px;">' +
                        ...
                        escHtml(cat.name) +
                        '</a>' +
                        '<button type="button" onclick="window.nexoDrillRight(' + cat.id + ')" ' +
        ...
        function nexoActivateCat(catId) {
            var tree = window.__nexoCatTree || [];
            var cat = tree.find(function (c) {
                return c.id == catId;
            });

            // Highlight active left-column button
            var leftCol = document.getElementById('nexoCatLeft');
            if (leftCol) {
        ...
        function nexoHandleCatClick(btn) {
        ...
        window.nexoActivateCat = nexoActivateCat;
        window.nexoHandleCatClick = nexoHandleCatClick;
        window.nexoDrillRight = nexoDrillRight;
        window.nexoRightBack = nexoRightBack;
    })();
</script>
