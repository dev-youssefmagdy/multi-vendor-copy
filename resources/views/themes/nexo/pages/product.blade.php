...
<main class="flex-grow bg-white pt-4 pb-14 w-full">
        ...
        <div class="flex flex-col lg:grid lg:grid-cols-2 gap-3 lg:gap-10">
            ...
            <div id="mantiProductMediaSlider" data-media-items='@json($mediaItems)' class="flex flex-col gap-4 lg:gap-6"
                ...
                <div class="relative select-none w-[500px] h-[500px] max-w-full mx-auto">
                    ...
                    <div class="swiper-button-prev w-10 h-10 rounded-full bg-white -translate-x-5 drop-shadow-2xl">
                        <svg class="!fill-white !w-[10px] rtl:rotate-180" width="7" height="14" viewBox="0 0 7 14" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path
            ...
            <div class="flex flex-col gap-3 lg:gap-6">
                ...
                <h1 class="text-[#222222] text-[24px] lg:text-[32px] font-medium leading-[24px] lg:leading-[28px] mb-3">
                    <button type="button" onclick="nexoOpenDeliveryModal()"
                ...
                <div class="flex gap-2 lg:gap-4 items-center -mt-4">
                    ...
                    <p id="nexo-weight-display" class="text-main text-base">{{ $weightDisplay }}</p>
                ...
                <div class="flex flex-wrap items-end gap-2.5">
                    <span class="text-[#222] text-[28px] lg:text-[32px] font-bold leading-none tracking-tight">
                        {{ $symbol }}<span id="nexo-sell-price">{{ $displaySell }}</span>
                    </span>
                    <span id="nexo-real-price"
                          class="{{ $hasDiscount ? '' : 'hidden' }} text-[#989898] text-[16px] font-medium line-through leading-none">
                        {{ $symbol }}<span id="nexo-real-price-val">{{ $displayReal }}</span>
                    </span>
                    <span id="nexo-discount-badge"
                          class="{{ $hasDiscount ? '' : 'hidden' }} bg-primary text-white text-[11px] font-bold px-2 py-1 rounded-md leading-none">
                        -<span id="nexo-discount-pct">{{ $discountPct }}</span>%
                    </span>
                </div>

                {{-- Stock badge --}}
                <div id="nexo-stock-badge" class="inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-[12px] font-semibold w-fit {{ $isInStock ? 'bg-[#e8fbe8] text-[#166534]' : 'bg-red-100 text-red-700' }}">
                    <span id="nexo-stock-dot" class="w-2 h-2 rounded-full {{ $isInStock ? 'bg-[#22c55e]' : 'bg-red-500' }}"></span>
                    <span id="nexo-stock-text">{{ $isInStock ? __('In Stock') : __('Out of Stock') }}</span>
                        ...
                        <div class="flex flex-wrap gap-2.5">
                            @foreach ($variants as $variant)
                                ...
                                <button type="button"
                                        ...
                                        onclick="nexoSelectVariant({{ $variant->id }})"
                ...
                <div class="hidden sm:flex items-center gap-3 mb-8">
                    <button id="nexo-add-to-cart-btn" type="button" onclick="nexoAddToCart()"
                        ...
                        <span id="nexo-add-to-cart-label">{{ __('Add to Cart') }}</span>
                    </button>
                    ...
                    <button type="button"
                        ...
                        <svg id="nexo-fav-icon-outline" class="w-3 h-3 sm:w-5 sm:h-5" viewBox="0 0 24 24" fill="none">
                            <path
                                d="M12.62 20.8101C12.28 20.9301 11.72 20.9301 11.38 20.8101C8.48 19.8201 2 15.6901 2 8.6901C2 5.6001 4.49 3.1001 7.56 3.1001C9.38 3.1001 10.99 3.9801 12 5.3401C13.01 3.9801 14.63 3.1001 16.44 3.1001C19.51 3.1001 22 5.6001 22 8.6901C22 15.6901 15.52 19.8201 12.62 20.8101Z"
                        ...
                        <svg id="nexo-fav-icon-filled" class="w-3 h-3 sm:w-5 sm:h-5 hidden" viewBox="0 0 24 24" fill="#FF4D00">
                            <path
                                d="M12.62 20.8101C12.28 20.9301 11.72 20.9301 11.38 20.8101C8.48 19.8201 2 15.6901 2 8.6901C2 5.6001 4.49 3.1001 7.56 3.1001C9.38 3.1001 10.99 3.9801 12 5.3401C13.01 3.9801 14.63 3.1001 16.44 3.1001C19.51 3.1001 22 5.6001 22 8.6901C22 15.6901 15.52 19.8201 12.62 20.8101Z"/>
                        </svg>
                    </button>
                    <!-- share button -->
                    <button type="button" onclick="nexoOpenShareModal()"
                ...
                <div
                    ...
                    <div
                        ...
                        <div
                            ...
                            <input id="nexo-qty-input-mobile" oninput="nexoSetQty(this.value)" type="number" min="1"
                                   value="{{ $qty }}"
                    ...
                    <button type="button"
                            ...
                            data-product-id="{{ $product->id }}"
                            aria-label="{{ __('Add to Favorites') }}"
                            class="w-11 h-11 bg-white rounded-full flex items-center justify-center flex-shrink-0 transition-transform hover:scale-110 border border-gray-200 shadow-sm">
                    ...
                    <button id="nexo-mobile-add-to-cart-btn" type="button" onclick="nexoAddToCart()"
                            @if (!$isInStock) disabled @endif
                        ...
                        <span id="nexo-mobile-add-to-cart-label">{{ __('Add to Cart') }}</span>
                    </button>
                </div>
                    ...
                    <div class="border border-[#EEEEEE] rounded-lg px-4 py-2 col-span-4 lg:col-span-2 order-2">
                        <h3 class="text-[14px] font-bold text-[#2AAF2F] mb-2 flex items-center gap-1">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="lg:hidden" onclick="nexoOpenPrivacyModal()"><svg width="7" height="12" viewBox="0 0 7 12" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div id="product-preview"
        ...
        <div class="relative select-none mb-8 w-full max-w-4xl h-[95%] mx-auto">
                    ...
                    @foreach($mediaItems as $idx => $item)
                        @if($item['type'] === 'video')
                            <div class="swiper-slide relative flex items-center justify-center bg-[#1a1a1a]">
                                <video
                                    class="w-full h-full object-contain nexo-fs-gallery-video"
                                    src="{{ $item['src'] }}"
            ...
            <div class="swiper-button-prev w-10 h-10 rounded-full bg-white !left-0 drop-shadow-2xl">
                <svg
                    class="!fill-white !w-[10px] rtl:rotate-180" width="7" height="14" viewBox="0 0 7 14" fill="none"
                    xmlns="http://www.w3.org/2000/svg">
    ...
    @if ($deliveryPopupDays->isNotEmpty())
        ...
        @endphp
        <div id="nexo-delivery-modal-backdrop"
             onclick="if(event.target===this)nexoCloseDeliveryModal()"
             class="hidden fixed inset-0 bg-[rgba(0,0,0,.45)] z-[100] items-center justify-center p-4"
             >

            <div id="nexo-delivery-modal"
                ...
                {{-- Close button --}}
                <button type="button" onclick="nexoCloseDeliveryModal()"
    ...
    {{-- ─────────────────────────── privacy&secure POPUP MODAL ─────────────────────────── --}}
        <div id="nexo-privacy-modal-backdrop"
             onclick="if(event.target===this)nexoClosePrivacyModal()"
             class="hidden fixed inset-0 bg-[rgba(0,0,0,.45)] z-[100] items-center justify-center p-4"
             >

            <div id="nexo-privacy-modal"
                ...
                {{-- Close button --}}
                <button type="button" onclick="nexoClosePrivacyModal()"
                        class="absolute -top-5 right-3 w-11 h-11 rounded-full bg-white flex items-center justify-center cursor-pointer z-10"
                       >
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
                        <path d="M1 1l12 12M13 1L1 13" stroke="#8F8F8F" stroke-width="1.8" stroke-linecap="round"/>
...
</span><span>{{ __('Why choose Nexo') }}</span></h2>
...
</span><p class="text-[10px]">{{ __("Nexo follows the Payment Card Industry (PCI DSS) data security standard and other security standards when handling card data")}}</p></li>
    <li class="flex items-center gap-2"><span><svg width="11" height="8" viewBox="0 0 11 8" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M9.774 0.75L3.987 6.536L0.75 3.304" stroke="#2AAF2F" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
...
</span><p class="text-[10px]">{{ __('Nexo does not sell your card information')}}</p></li>
 </ul>
...
<span class="text-main mt-2">{{ __('https://hackerone.com/Nexo')}}</span>
</p>
            </div>
        </div>
    {{-- ── Share Modal ─────────────────────────────────────────────────────────── --}}
    <div id="nexo-share-modal"
         class="hidden fixed inset-0 z-[9999] flex items-end sm:items-center justify-center"
         onclick="if(event.target===this) nexoCloseShareModal()">
        {{-- Backdrop --}}
        <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>

        {{-- Sheet --}}
        <div id="nexo-share-sheet"
            ...
            <div class="flex items-center justify-between mb-5">
                ...
                <button onclick="nexoCloseShareModal()"
            ...
            <div class="grid grid-cols-4 gap-3 mb-6">
                {{-- WhatsApp --}}
                <a id="nexo-share-whatsapp" href="#" target="_blank" rel="noopener noreferrer"
                ...
                {{-- Facebook --}}
                <a id="nexo-share-facebook" href="#" target="_blank" rel="noopener noreferrer"
                ...
                {{-- X / Twitter --}}
                <a id="nexo-share-twitter" href="#" target="_blank" rel="noopener noreferrer"
                ...
                {{-- Telegram --}}
                <a id="nexo-share-telegram" href="#" target="_blank" rel="noopener noreferrer"
            ...
            <div class="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5">
                ...
                <span id="nexo-share-url-text" class="flex-1 text-sm text-gray-600 truncate select-all"></span>
                <button id="nexo-copy-btn" onclick="nexoCopyShareUrl()"
...
@push('scripts')
@vite('resources/js/nexo/product.js')
    <script>
        window.__nexoProductConfig = {
    ...
    <script>
        ...
        const NEXO_CART_ADD_URL = @json($cartAddUrl);
        const NEXO_PRODUCT_SLUG = @json($product->slug);
        let nexoSelectedVariantId = @json($activeVariant?->id);
        let nexoQty = @json($qty);

        function nexoFormatWeight(grams) {
            if (!grams) return '';
            return grams >= 1000
        ...
        window.storefrontUpdateShippingProgress = function (data) {
            ...
            const message = remaining <= 0
                ...
                : @json(__('Add :weight more to qualify for free shipping', ['weight' => '__WEIGHT__'])).replace('__WEIGHT__', nexoFormatWeight(remaining));

            ['', '-mobile'].forEach(function (suffix) {
                const bar = document.getElementById('nexo-shipping-bar' + suffix);
                if (bar) bar.style.width = pct + '%';
                const msg = document.getElementById('nexo-shipping-message' + suffix);
                if (msg) msg.textContent = message;
            });
        };

        function nexoSetQty(value) {
            nexoQty = Math.max(1, parseInt(value, 10) || 1);
            nexoSyncQtyInputs();
        }

        function nexoIncrementQty() {
            nexoQty++;
            nexoSyncQtyInputs();
        }

        function nexoDecrementQty() {
            if (nexoQty > 1) nexoQty--;
            nexoSyncQtyInputs();
        }

        function nexoSyncQtyInputs() {
            ['nexo-qty-input', 'nexo-qty-input-mobile'].forEach(function (id) {
                const el = document.getElementById(id);
                if (el) el.value = nexoQty;
            });
        }

        function nexoAddToCart() {
            const buttons = ['nexo-add-to-cart-btn', 'nexo-mobile-add-to-cart-btn']
                .map((id) => document.getElementById(id))
                .filter(Boolean);
            const labels = ['nexo-add-to-cart-label', 'nexo-mobile-add-to-cart-label']
                .map((id) => document.getElementById(id))
                .filter(Boolean);
            ...
            window.storefrontCartAdd({
                url: NEXO_CART_ADD_URL,
                slug: NEXO_PRODUCT_SLUG,
                variantId: nexoSelectedVariantId,
                qty: nexoQty,
            }).then(function () {
                // ── Tracking: AddToCart ───────────────────────────────────────────
                if (typeof window.trackAddToCart === 'function') {
                    var variant = nexoSelectedVariantId ? NEXO_VARIANTS[nexoSelectedVariantId] : null;
        ...
        function nexoSelectVariant(id) {
            const data = NEXO_VARIANTS[id];
            if (!data) return;
            if (!data.isInStock) {
            ...
            const titleEl = document.getElementById('nexo-variant-title');
            if (titleEl) titleEl.textContent = data.title;

            // Update sell price
            const sellEl = document.getElementById('nexo-sell-price');
            if (sellEl) sellEl.textContent = data.displaySell;

            // Update discount area
            const realEl = document.getElementById('nexo-real-price');
            const realVal = document.getElementById('nexo-real-price-val');
            const badgeEl = document.getElementById('nexo-discount-badge');
            const pctEl = document.getElementById('nexo-discount-pct');
            ...
            // Update stock badge
            const stockBadge = document.getElementById('nexo-stock-badge');
            const stockDot = document.getElementById('nexo-stock-dot');
            const stockText = document.getElementById('nexo-stock-text');
            if (stockBadge) {
                if (data.isInStock) {
            ...
            ['nexo-add-to-cart-btn', 'nexo-mobile-add-to-cart-btn'].forEach(function (btnId) {
                const btn = document.getElementById(btnId);
                if (btn) btn.disabled = !data.isInStock;
            });

            // Update weight
            const weightEl = document.getElementById('nexo-weight-display');
            if (weightEl) weightEl.textContent = data.weightDisplay ?? '';

            // Jump the main gallery to this variant's image, if it has one
            if (data.mediaIndex !== null && data.mediaIndex !== undefined && window.nexoGoToMediaIndex) {
                window.nexoGoToMediaIndex(data.mediaIndex);
            }
        }
    </script>
    <script>
