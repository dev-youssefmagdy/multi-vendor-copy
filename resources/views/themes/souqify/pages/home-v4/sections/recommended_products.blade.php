...
@endphp
<!-- ============ RECOMMENDED FOR YOU ============ -->
<section class="px-[16px] lg:px-[56px] py-[24px] lg:py-[48px] flex flex-col gap-[16px] lg:gap-[28px]" style="background: linear-gradient(120deg, color-mix(in srgb, var(--color-brand-green) 8%, var(--color-bg-main)), color-mix(in srgb, var(--color-badge-yellow) 8%, var(--color-bg-main)))">
  ...
  <div id="recommendedGrid" class="grid grid-cols-2 lg:grid-cols-5 gap-[12px] lg:gap-[16px]">
    @forelse ($__recommendedCards as $p)
      <div class="h-[380px] lg:h-[440px]" wire:key="recommended-v4-{{ $p['id'] }}">
        @include('themes.souqify.pages.home-v2.sections.partials.product_card', ['p' => $p])
      </div>
    @empty
      <p class="text-sm py-6 col-span-full" style="color:var(--color-footer-text-muted)">{{ __('No recommended products yet.') }}</p>
    @endforelse
  </div>
