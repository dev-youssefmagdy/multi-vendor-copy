...
<style>
    ...
    [dir="rtl"] .sqv2-deal__tag {
        left: auto;
        right: 0;
        border-radius: 0 0 0 9cqw;
    }
    [dir="rtl"] .sqv2-deal__fav {
        right: auto;
        left: 2.79cqw;
    }
    [dir="rtl"] .sqv2-deal__cart {
        left: auto;
        /* Exact mirror of `left: 73.75%` for a 23.75cqw-wide button: both terms
           are relative to the container's inline-size, so this stays exact at
           any card width (equivalent to ~2.5cqw from the edge). */
        right: calc(26.25% - 23.75cqw);
    }
    [dir="rtl"] .sqv2-deal__ribbon {
        left: auto;
        right: 0;
        padding: 1.25cqw 2.5cqw 1.25cqw 8cqw;
...
<div class="sqv2-deal">
    <a href="{{ $p['url'] }}" class="sqv2-deal__media">
        ...
        <span class="sqv2-deal__cart" role="button" tabindex="0" aria-label="{{ __('Add to cart') }}"
    ...
    @if (!empty($p['slug']) && !empty($p['favData']))
        <button type="button" onclick="souqifyToggleFavorite(this)" data-slug="{{ $p['slug'] }}" data-fav='{{ $p['favData'] }}' aria-label="{{ __('Add to favorites') }}" class="sqv2-deal__fav">
