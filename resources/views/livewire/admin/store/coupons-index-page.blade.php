<main id="mn">
    ...
    <div class="fu d1 section-gap" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:16px;">
        <a href="{{ route('admin.store.coupons.list') }}" class="card" style="display:block;text-decoration:none;">
            <h3 class="panel-title">Default</h3>
            <p class="panel-copy">Synced to every tenant that has no country-specific coupons.</p>
            <span class="badge mt-2">{{ $defaultCount }} {{ Str::plural('coupon', $defaultCount) }}</span>
        </a>

        @foreach ($countries as $country)
            <a href="{{ route('admin.store.coupons.list', ['countryId' => $country->id]) }}" class="card" style="display:block;text-decoration:none;">
