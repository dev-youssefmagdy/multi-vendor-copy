<main id="mn">
    ...
    <div class="content-grid section-gap">
        <div class="card fu d1">
            <h3 class="panel-title">Page Scope</h3>
            ...
            <div class="content-note-stack">
                ...
                @else
                    <div class="content-note">
                        <strong>Livewire only.</strong>
                        This scaffold avoids Alpine and is ready for service-backed actions.
                    </div>
                    <div class="content-note">
                        <strong>Next step.</strong>
                        Connect enums, repositories, validation, and alert flows page by page.
                    </div>
                @endif
            </div>
        </div>
                        ...
                        @foreach ($group['fields'] as $field)
                            <div class="{{ $field['wrapperClass'] ?? '' }}">
                                ...
                                @elseif (($field['type'] ?? 'text') === 'file')
                                    <x-dropzone id="{{ $field['model'] }}-upload" style="width:100%"
                                        model="{{ $field['model'] }}" :default="$field['preview'] ?? null" :multiple="false"
                                        accept="image/*" label="{{ $field['uploadLabel'] ?? 'Upload image' }}"
                                        sublabel="{{ $field['uploadSublabel'] ?? 'PNG, JPG, SVG, WEBP up to 2MB' }}" />
                                @else
                        ...
                        <tr>
                            <td colspan="{{ count($section['headers']) }}">
                                <div class="empty-state">
                                    ...
                                    <p class="empty-state-copy">This central reporting section does not have enough records yet.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
