<main id="mn">
    ...
    <div class="page-head fu d0">
        <div>
            <h1 class="D page-title">Blade Themes</h1>
            <p class="page-copy">Review, approve, or reject tenant-uploaded Blade storefront themes before they can go live. Approving a theme does not activate it — the vendor still has to activate it from their dashboard.</p>
        </div>
    </div>
            ...
            <table class="ct-table">
                ...
                <tbody>
                    @forelse($themes as $theme)
                        <tr>
                            <td>{{ $theme->tenant?->name ?? $theme->tenant_id }}</td>
                            <td>{{ $theme->version }}</td>
                            ...
                            <td class="ct-actions">
                                @if($theme->status === 'pending')
                                    <button type="button" wire:click="requestApprove({{ $theme->id }})" class="ct-link">Approve</button>
                                    <button type="button" wire:click="openReject({{ $theme->id }})" class="ct-link ct-link-danger">Reject</button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr><td colspan="5">No themes found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div style="margin-top:16px;">{{ $themes->links() }}</div>
    </section>

    @if($rejectModalOpen)
        <div class="ct-modal-backdrop" wire:click.self="closeReject">
            <div class="ct-modal">
                <h3 class="panel-title">Reject theme</h3>
                ...
                <div class="gs-actions" style="margin-top:16px;">
                    <x-btn type="button" wire:click="reject">Reject theme</x-btn>
