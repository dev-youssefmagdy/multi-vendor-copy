<main id="mn">
    ...
    {{-- ══════════════════════════════════════════════════════════════════════
         Section 1 — Installed languages
    ══════════════════════════════════════════════════════════════════════ --}}
    <section class="card table-card-shell section-gap fu d4">
        <div class="table-header-shell">
            <div>
                <h3 class="panel-title">Installed Languages</h3>
                <p class="panel-copy">Languages currently available in your store. Activate, disable, or set the default locale.</p>
            </div>
        </div>

        @if (count($installedRows) > 0)
            <x-table :headers="['Language', 'Direction', 'Countries', 'Type', 'Status', 'Actions']">
                @foreach ($installedRows as $row)
                    <tr wire:key="installed-{{ $row['id'] }}">
                        <td>
                            <div style="display:flex;align-items:center;gap:10px;">
                                @if ($row['imageUrl'])
                                    <img src="{{ $row['imageUrl'] }}" alt="{{ $row['name'] }}"
                                         style="width:36px;height:24px;object-fit:cover;border-radius:4px;flex-shrink:0;" />
                                @endif
                                <div>
                                    <div class="entity-title">{{ $row['name'] }}</div>
                                    <div class="entity-subtitle">{{ $row['nativeName'] }} · {{ strtoupper($row['code']) }}</div>
                                </div>
                            </div>
                        </td>
                        <td>{{ $row['direction'] }}</td>
                        ...
                        <td>
                            <div class="flex gap-2">
                                ...
                                @unless ($row['isDefault'])
                                    <button type="button" class="btn btn-secondary btn-sm"
                                            wire:click="makeDefault({{ $row['id'] }})">
                                        Make Default
                                    </button>
                                @endunless
                            </div>
        ...
        @else
            <div class="empty-state">
                <div class="empty-state-title">No languages installed</div>
                <p class="empty-state-copy">No languages have been set up in your tenant store yet.</p>
            </div>
        @endif
    </section>

    {{-- ══════════════════════════════════════════════════════════════════════
         Section 2 — Available for purchase
    ══════════════════════════════════════════════════════════════════════ --}}
    @if ($availableLanguages->isNotEmpty())
        <section class="card table-card-shell section-gap fu d5">
            <div class="table-header-shell">
                <div>
                    <h3 class="panel-title">Available Language Add-ons</h3>
                    <p class="panel-copy">Purchase a language once to unlock it permanently for your storefront and admin panel.</p>
                </div>
            </div>

            @if (session('purchase_error'))
                <div class="notice-error" style="margin:0 22px 16px;">{{ session('purchase_error') }}</div>
            @endif
            @if ($errors->has('payment'))
                <div class="notice-error" style="margin:0 22px 16px;">{{ $errors->first('payment') }}</div>
            @endif

            <x-table :headers="['Language', 'Direction', 'Countries', 'Price', 'Action']">
                @foreach ($availableLanguages as $language)
                    <tr wire:key="available-{{ $language->id }}">
                        <td>
                            <div style="display:flex;align-items:center;gap:10px;">
                                ...
                                <div>
                                    ...
                                    <div class="entity-subtitle">{{ $language->native_name }} · {{ strtoupper($language->code) }}</div>
                                </div>
                            </div>
                        </td>
        ...
        <x-modal wire:model="showPaymentModal"
            ...
            <div class="page-stack">
                    ...
                    @if ($gateways->isNotEmpty())
                        <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px;">
                            @foreach ($gateways as $gateway)
                                <label wire:key="lm-gw-{{ $gateway['id'] }}"
                        ...
                        @else
                            <div class="form-grid form-grid-1" style="gap:10px;">
                                <div>
                                    ...
                                    <x-input type="text" id="lm-card-number" inputmode="numeric" autocomplete="cc-number"
                                             placeholder="1234  5678  9012  3456" />
                                </div>
                                <div class="form-grid form-grid-2" style="gap:10px;">
                                    <div>
                                        <label class="field-label">Expiry</label>
                                        <x-input type="text" id="lm-card-expiry" inputmode="numeric" autocomplete="cc-exp"
                                                 placeholder="MM / YY" maxlength="7" />
                                    </div>
                                    <div>
                                        <label class="field-label">CVC</label>
                                        <x-input type="text" id="lm-card-cvc" inputmode="numeric" autocomplete="cc-csc"
                            ...
                            @endif
                            <p id="lm-card-errors" class="field-error" style="display:none;"></p>
                        @endif
                    </div>
                @endif
                ...
                <div class="page-actions compact-actions justify-end">
                    ...
                    @if ($gateways->isNotEmpty())
                        <x-btn type="button" id="lm-pay-btn"
    ...
    <script>
        let _lmStripe     = null;
        let _lmStripeCard = null;

        function lmInitPaymentForm() {
            const container = document.getElementById('lm-inline-card-form');
            ...
            if (gateway === 'stripe') {
                ...
                _lmStripe     = Stripe(stripeKey);
                const elms    = _lmStripe.elements();
                _lmStripeCard = elms.create('card', {
                    style: {
                        base: {
                            fontSize: '13px',
                            color: '#333333',
                ...
                _lmStripeCard.mount('#lm-stripe-card-element');
                _lmStripeCard.on('change', (e) => {
                    const errEl = document.getElementById('lm-stripe-card-errors');
                    if (errEl) {
                ...
                lmSetupCardFormatting();
                container.dataset.initialized = '1';
            }
        }

        function lmSetupCardFormatting() {
            const numInput = document.getElementById('lm-card-number');
            ...
            const expInput = document.getElementById('lm-card-expiry');
        ...
        Livewire.on('buyLanguagePaymentMethodChanged', () => {
            setTimeout(lmInitPaymentForm, 30);
        });
        ...
        document.addEventListener('click', async (event) => {
            const button = event.target.closest('#lm-pay-btn');
            if (!button) return;

            const inlineForm = document.getElementById('lm-inline-card-form');
            ...
            // ── Stripe ────────────────────────────────────────────────────
            if (gateway === 'stripe') {
                if (!_lmStripe || !_lmStripeCard) {
                    alert('Stripe.js is still loading. Please try again in a moment.');
                    button.disabled = false;
                    return;
                }
                const { token, error } = await _lmStripe.createToken(_lmStripeCard);
                if (error) {
                    const errEl = document.getElementById('lm-stripe-card-errors');
            ...
            if (gateway === 'authorize_net') {
                ...
                const rawExp  = document.getElementById('lm-card-expiry')?.value.replace(/\s/g, '') ?? '';
                ...
                const secureData = {
                    ...
                    cardData: {
                        cardNumber: document.getElementById('lm-card-number')?.value.replace(/\s/g, '') ?? '',
                        month:      (mon ?? '').trim(),
                        year:       '20' + (yr ?? '').trim(),
                        cardCode:   document.getElementById('lm-card-cvc')?.value ?? '',
                    },
                };
                Accept.dispatchData(secureData, async (response) => {
                    if (response.messages.resultCode === 'Error') {
                        const errEl = document.getElementById('lm-card-errors');
            ...
            if (gateway === '2checkout') {
                ...
                TCO.requestToken({
                    ...
                    ccNo:           document.getElementById('lm-card-number')?.value.replace(/\s/g, '') ?? '',
                    cvv:            document.getElementById('lm-card-cvc')?.value ?? '',
                    expMonth:       (mon ?? '').trim(),
                    expYear:        '20' + (yr ?? '').trim(),
                }, async (data) => {
                    if (data.errorCode > 0) {
