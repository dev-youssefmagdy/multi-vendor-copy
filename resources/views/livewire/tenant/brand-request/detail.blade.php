<main id="mn">
    <div class="page-head fu d0">
        <div>
            ...
            <p class="page-copy">Brand Request #{{ $request->id }} &middot; Submitted {{ $request->created_at->format('M d, Y H:i') }}</p>
        </div>
        <div class="page-actions">
            <a href="{{ route('tenant.brand-requests.index') }}" class="btn btn-secondary">← Back to Requests</a>
        </div>
    </div>
        ...
        <x-modal wire:model="showPaymentModal"
            ...
            <div class="page-stack">
                ...
                <div class="locale-fields-group" style="margin-bottom:4px;">
                    <div style="display:flex;align-items:center;justify-content:space-between;gap:14px;">
                        <div>
                            ...
                            <div class="entity-subtitle">Brand Request #{{ $request->id }} · {{ $request->title }}</div>
                        </div>
                    ...
                    @if ($gateways->isNotEmpty())
                        <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px;">
                            @foreach ($gateways as $gateway)
                                <label wire:key="br-gw-{{ $gateway['id'] }}"
                        ...
                        @else
                            <div class="form-grid form-grid-1" style="gap:10px;">
                                <div>
                                    ...
                                    <x-input type="text" id="br-card-number" inputmode="numeric" autocomplete="cc-number"
                                             placeholder="1234  5678  9012  3456" />
                                </div>
                                <div class="form-grid form-grid-2" style="gap:10px;">
                                    <div>
                                        <label class="field-label">Expiry</label>
                                        <x-input type="text" id="br-card-expiry" inputmode="numeric" autocomplete="cc-exp"
                                                 placeholder="MM / YY" maxlength="7" />
                                    </div>
                                    <div>
                                        <label class="field-label">CVC</label>
                                        <x-input type="text" id="br-card-cvc" inputmode="numeric" autocomplete="cc-csc"
                            ...
                            @endif
                            <p id="br-card-errors" class="field-error" style="display:none;"></p>
                        @endif
                    </div>
                @endif
                ...
                <div class="page-actions compact-actions justify-end">
                    ...
                    @if ($gateways->isNotEmpty())
                        <x-btn type="button" id="br-pay-btn"
    ...
    <script>
        let _brStripe     = null;
        let _brStripeCard = null;

        function brInitPaymentForm() {
            const container = document.getElementById('br-inline-card-form');
            ...
            if (gateway === 'stripe') {
                ...
                _brStripe = Stripe(stripeKey);
                const elms = _brStripe.elements();
                ...
                _brStripeCard = elms.create('card', {
                    style: {
                        base: {
                            fontSize: '13px',
                            color: computedColor,
                ...
                _brStripeCard.mount('#br-stripe-card-element');
                _brStripeCard.on('change', (e) => {
                    const errEl = document.getElementById('br-stripe-card-errors');
                    if (errEl) {
                ...
                brSetupCardFormatting();
                container.dataset.initialized = '1';
            }
        }

        function brSetupCardFormatting() {
            const numInput = document.getElementById('br-card-number');
            if (numInput) {
            ...
            const expInput = document.getElementById('br-card-expiry');
            if (expInput) {
        ...
        $wire.on('brPaymentMethodChanged', () => {
            _brStripe     = null;
            _brStripeCard = null;
            setTimeout(brInitPaymentForm, 50);
        });

        setTimeout(brInitPaymentForm, 50);

        document.addEventListener('click', async (e) => {
            const button = e.target.closest('#br-pay-btn');
            if (!button) return;
            ...
            const inlineForm = document.getElementById('br-inline-card-form');
            ...
            if (gateway === 'stripe') {
                if (!_brStripe || !_brStripeCard) {
                    alert('Stripe is still initialising. Please wait a moment.');
                    button.disabled = false;
                    return;
                }
                const { token, error } = await _brStripe.createToken(_brStripeCard);
                if (error) {
                    const errEl = document.getElementById('br-stripe-card-errors');
            ...
            if (gateway === 'authorize_net') {
                ...
                const rawExp = document.getElementById('br-card-expiry')?.value.replace(/\s/g, '') ?? '';
                ...
                const secureData = {
                    ...
                    cardData: {
                        cardNumber: document.getElementById('br-card-number')?.value.replace(/\s/g, '') ?? '',
                        month:      (mon ?? '').trim(),
                        year:       '20' + (yr ?? '').trim(),
                        cardCode:   document.getElementById('br-card-cvc')?.value ?? '',
                    },
                };
                Accept.dispatchData(secureData, async (response) => {
                    if (response.messages.resultCode === 'Error') {
                        const errEl = document.getElementById('br-card-errors');
            ...
            if (gateway === '2checkout') {
                ...
                TCO.requestToken({
                    ...
                    ccNo:           document.getElementById('br-card-number')?.value.replace(/\s/g, '') ?? '',
                    cvv:            document.getElementById('br-card-cvc')?.value ?? '',
                    expMonth:       (mon ?? '').trim(),
                    expYear:        '20' + (yr ?? '').trim(),
                }, async (data) => {
                    if (data.errorCode > 0) {
    ...
    <script>
        if (window.Echo) {
            window.Echo.private('tenant.{{ tenant("id") }}.brand-requests.{{ $requestId }}')
