<div class="form-grid form-grid-2">
        ...
        @if (!empty($productIds))
            <div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:8px;">
                @foreach ($productIds as $selectedId)
                    ...
                    <span
                        style="display:inline-flex;align-items:center;gap:6px;padding:3px 8px 3px 4px;border-radius:999px;background:rgba(0,229,255,.12);color:var(--cyan);font-size:11.5px;font-weight:700;">
                        @if (!empty($selectedProductImages[$intId]))
                            <img src="{{ $selectedProductImages[$intId] }}"
                                style="width:22px;height:22px;border-radius:50%;object-fit:cover;flex-shrink:0;border:1px solid rgba(0,229,255,.3);"
                                alt="">
                        @endif
        ...
        <div style="border:1px solid var(--border);border-radius:8px;margin-top:4px;max-height:220px;overflow-y:auto;position:relative;"
            ...
            @forelse ($productSearchResults as $productId => $productName)
                ...
                <button type="button" class="enhanced-select-option {{ $isSelected ? 'is-selected' : '' }}"
                    ...
                    @if (!empty($productSearchImages[$productId]))
                        <img src="{{ $productSearchImages[$productId] }}"
                            style="width:28px;height:28px;border-radius:6px;object-fit:cover;flex-shrink:0;margin-right:4px;"
                            alt="">
                    @endif
    ...
    {{-- Banner --}}
    <div>
        <label class="field-label">Banner Image <span style="font-weight:400;color:var(--t3);">(optional)</span></label>
        <x-input type="file" wire:model="bannerImage" :error="$errors->has('bannerImage')" />
        @error('bannerImage')<div class="field-error">{{ $message }}</div>@enderror
        @php $previewUrl = $bannerImage ? $bannerImage->temporaryUrl() : ($bannerImagePath ?? null); @endphp
        @if ($previewUrl)
            <img src="{{ $previewUrl }}"
                style="margin-top:8px;width:100%;max-height:80px;object-fit:cover;border-radius:8px;border:1px solid var(--border);"
                alt="Banner preview">
        @endif
    </div>
