...
class NewInPage extends Component
{
    ...
    public function render()
    {
        ...
        $data = array_merge($this->sharedData(), [
            'products' => $repo->paginatedProductsByBadge('new-in', [
                'keyword' => $this->search,
        ...
        return view($this->pageView('new-in'), $data)
            ->layout($this->storefrontLayout(), [
                'title' => $storeName ? __('New In') . " — {$storeName}" : __('New In'),
                'metaDescription' => $storeName ? __('Discover the latest new arrivals at :store.', ['store' => $storeName]) : __('Discover the latest new arrivals.'),
                'canonicalUrl' => route('tenant.storefront.new-in'),
            ]);
    }
}
